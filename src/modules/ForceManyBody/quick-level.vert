...
attribute vec2 idxVec;

varying vec4 velocity;

void main() {
  // float idx = idxVec.x + idxVec.y * inputSize;
  // float iandj = floor(idx / nodeslen);
  // float idxpart = idx - iandj * nodeslen;

  // float idxy = floor(idxpart/positionTextureSize);
  // vec2 index = vec2(0.0, 0.0);
  // index.x = idxpart/positionTextureSize - idxy;
  // index.y = idxy / positionTextureSize;

  vec2 index = idxVec / positionTextureSize;

  vec4 pointPosition = texture2D(position, index);
  float x = pointPosition.x;
  float y = pointPosition.y;

  velocity = vec4(vec2(0.0), 1.0, 0.0);
    ...
    // float i = floor(iandj / 3.0);
    // float j = iandj - floor(iandj / 3.0) * 3.0;
  ...
  // }
  gl_Position = vec4( index*2.0 - 1.0, 0.0, 1.0 );
  gl_PointSize = 1.0;
